...
@Component({
  selector: 'app-newsession-admin',
  templateUrl: './newsession-admin.component.html',
  styleUrls: ['./newsession-admin.component.css']
})
export class NewsessionAdminComponent {
  sesiones: any[];  
  sesionesf: any;
  sesionee = {
    nombreAlumno: '',
  ...
  ngOnInit(): void {
    ...
    this.av.getAll().subscribe((data) => {
      // Filtra solo las sesiones de tipo "Nueva Sesión"
      this.sesiones = data.filter((sesion) => sesion.data.tipo === 'Nueva Sesión');
      console.log(this.sesiones);
    });
  }
