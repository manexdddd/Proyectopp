...
@Component({
  ...
  styleUrls: ['./followup-admin.component.css']
})
export class FollowupAdminComponent {
  sesiones: any[];
  sesionesf: any;
  sesionee = {
    nombreAlumno: '',
  ...
  ngOnInit(): void {
    ...
    this.av.getAll().subscribe((data) => {
      // Filtra solo las sesiones de tipo "Seguimiento"
      this.sesiones = data.filter((sesion) => sesion.data.tipo === 'Seguimiento');
      console.log(this.sesiones);
    });
  }
